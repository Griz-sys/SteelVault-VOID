...
export async function POST(req: Request) {
  ...
  const { filename, contentType, clientId, projectId, packageId, packageName } = await req.json();
    ...
    // NOTE: omit signing the Content-Type header to avoid SignatureDoesNotMatch
    // errors when the browser or intermediaries change or normalize the header
    // (for example adding a charset). The client should still set Content-Type
    // when uploading, but it will not be included in the signed string.
